...
ffmpeg -v error -y -i "$tmp" -vf "fps=60,scale=trunc(iw/2)*2:trunc(ih/2)*2" -r 60 -fps_mode cfr \
  ...
  -movflags +faststart -an "$out" && rm -f "$tmp"
ffprobe -v error -select_streams v -show_entries stream=codec_name,profile,avg_frame_rate,width,height -of default=nw=1 "$out"
